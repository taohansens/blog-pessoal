package br.com.taohansen.blog.repository;

import br.com.taohansen.blog.models.*;
import br.com.taohansen.blog.services.PostMapper;
...
/**
 * Serviço responsável pela comunicação com o CouchDB API.
...
public class CouchDbRepository {
     ...
     * A ordenação é feita pela view do CouchDB (descending), otimizando desempenho.
     *
    ...
    public Mono<Post> getPostBySlug(String slug, boolean includeDrafts) {
        ...
        return couchDbWebClient.get()
                ...
                .flatMapMany(response -> Flux.fromIterable(response.getRows())
                        .filter(row -> row.getDoc() != null)
                        .map(postMapper::mapToPost)
                        .filter(post -> sanitizedSlug.equals(post.getSlug()))
                        .filter(post -> {
                            // Filtrar rascunhos se não for admin
                            return includeDrafts || !Boolean.TRUE.equals(post.getDraft());
                        }))
                .next()
    ...
    private Mono<Post> searchPostBySlugInAllPosts(String slug, boolean includeDrafts) {
        ...
        return couchDbWebClient.get()
                ...
                .flatMapMany(response -> Flux.fromIterable(response.getRows())
                        ...
                        .filter(post -> includeDrafts || !Boolean.TRUE.equals(post.getDraft())))
                .next()
                .switchIfEmpty(Mono.defer(() -> {
        ...
        return couchDbWebClient.get()
                ...
                .map(response -> {
                    ...
                    List<PostMetadata> metadataList = allMetadata.stream()
                            .filter(meta -> includeDrafts || !Boolean.TRUE.equals(meta.getDraft()))
                            .toList();
    ...
    public Mono<Post> getPostById(String id) {
        ...
        return couchDbWebClient.get()
                ...
                .map(postMapper::mapDocumentToPost)
                .filter(obj -> true)
